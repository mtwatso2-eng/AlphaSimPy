...
import pytest
import numpy as np
from AlphaSimPy import as_categorical


def test_as_categorical_converts_correctly():
    """Test as_categorical function converts correctly"""
    cont = np.zeros((7, 3))
    cont[:, 0] = np.array([-3, -2, -1, 0, 1, 2, 3])
    cont[:, 1] = np.array([-3, -2, -1, 0, 1, 2, 3])
    cont[:, 2] = np.array([-3, -2, -1, 0, 1, 2, 3])

    result1 = as_categorical(x=cont[:, 0])
    expected1 = np.array([[1], [1], [1], [2], [2], [2], [2]])
    np.testing.assert_array_equal(result1, expected1)

    result2 = as_categorical(x=cont[:, 0], threshold=np.array([-1, 0, 1]))
    ...
    with pytest.raises((ValueError, TypeError)):
        as_categorical(x=cont, p=np.array([0.5, 0.5]))
    
    p_list = [None, np.array([0.5, 0.5]), None]
    with pytest.raises((ValueError, TypeError)):
        as_categorical(x=cont, p=p_list)
    
    with pytest.raises((ValueError, TypeError)):
        as_categorical(x=cont, p=p_list, mean=trt_mean)
    
    cont2 = as_categorical(x=cont, p=p_list, mean=trt_mean, var=trt_var)
    cont2_exp = cont.copy()
    cont2_exp[:, 1] = np.array([1, 1, 1, 2, 2, 2, 2])
    np.testing.assert_array_equal(cont2, cont2_exp)
