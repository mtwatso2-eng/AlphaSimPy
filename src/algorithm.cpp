...
#ifdef ALPHASIMPY_PYTHON
            std::cerr<<"proposed grandMRCA != top Node\n";
#else
    ...
    if (bUserEventAvailable &&
      ...
      if (eventType==Event::PAST_COAL){
        ...
        if ((iSourcePop>=iTotalPops||
            iDestPop>=iTotalPops)) {
#ifdef ALPHASIMPY_PYTHON
          std::cerr<<"Invalid past migration event at time "
#else
...
#ifdef ALPHASIMPY_PYTHON
          std::cerr<<"Error in specifying new migration matrix event.  The dimension of the matrix "<<dMigrationMatrix.size()<<" must equal the number of populations: "<<pPopList.size()<<endl;
#else
        ...
        if (iSourcePop>=iTotalPops){
#ifdef ALPHASIMPY_PYTHON
          std::cerr <<"Source pop and total pops are "<<iSourcePop<<","<<iTotalPops<<" in POP JOIN event at history "<<iGraphIteration<<". It is recommended that you increase the migration rates and/or number of sampled chromosomes.\n";
#else
...
#ifdef ALPHASIMPY_PYTHON
          std::cerr <<"Dest pop and total pops are "<<iDestPop<<","<<iTotalPops<<" in POP JOIN event\n";
#else
...
#ifdef ALPHASIMPY_PYTHON
        std::cerr<<"Found an event of type "<<pNextNewEvent->getType()
#else
...
#ifdef ALPHASIMPY_PYTHON
        if (*currentEventIt==NULL) std::cerr<<"null\n";
#else
...
#ifdef ALPHASIMPY_PYTHON
      std::cerr<<"startPos is "<<startPos<<" endPos is "
#else
