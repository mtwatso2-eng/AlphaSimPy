...
#else
#include <Rcpp.h>
#define RCPP_STOP(msg) Rcpp::stop(msg)
...
GenericEvent::GenericEvent(EventType iType,double dTime,
  ...
  //RCPP_RCERR<<"destroying generic event at height "<<dTime<<endl;
}
...
void GraphBuilder::checkPopCountIntegrity(PopVector & pPopList,
  ...
  for (int j=0;j<iTotalPops;++j){
    if (pPopList[j].getChrSampled()!=iCounts[j]){
      RCPP_RCERR<<"At time: "<<dTime<<endl;
      RCPP_RCERR<<"pop:"<<j<<",size:"<<pPopList[j].getChrSampled()<<endl;
      RCPP_RCERR<<"pop:"<<j<<",found:"<<iCounts[j]<<endl;
      printDataStructures();
...
void GraphBuilder::printHaplotypes(){
  ...
  if (iTotalSites){
    ...
    if (pConfig->bSNPAscertainment){
      ...
      if (bZeroCellCount){
        RCPP_RCERR<<"Warning: Some observed SNP counts were zero when they should have been positive.\n"<<
        ...
        iReducedSites = tally;
        RCPP_RCERR<<"Total sites reduced from "<<iTotalSites<<" to "<<iReducedSites<<endl;
      }
    }
    // if (iReducedSites){
      // MutationPtrVector::iterator it;
...
void GraphBuilder::printDataStructures(){
  RCPP_RCERR<<endl<<"*** Begin printing structures ***"<<endl;
  
  RCPP_RCERR<<"Full ARG (list of edges)\n";
  for (EdgePtrList::iterator it=pEdgeListInARG->begin();it!=pEdgeListInARG->end();it++){
    EdgePtr curEdge = *it;
    RCPP_RCERR<<"low:ht:"<<curEdge->getBottomNodeRef()->getHeight()<<
  ...
  RCPP_RCERR<<"Last tree (list of edges)\n";
  ...
  while(count<iTotalTreeEdges){
    ...
    EdgePtr curEdge = *it;
    RCPP_RCERR<<"low_ht:"<<curEdge->getBottomNodeRef()->getHeight()<<
      ",type:"<<curEdge->getBottomNodeRef()->getTypeStr()<<
        ",pop:"<<curEdge->getBottomNodeRef()->getPopulation()<<
  ...
  RCPP_RCERR<<"MRCA: "<<localMRCA->getHeight()<<endl;
  RCPP_RCERR<<"Graph grandMRCA: "<<grandMRCA->getHeight()<<endl;
  //    RCPP_RCERR<<"true length:"<<trueLen<<endl;
  
  RCPP_RCERR<<"*** Done printing structures ***"<<endl;
}
