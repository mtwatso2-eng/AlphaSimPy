#ifdef ALPHASIMPY_PYTHON
#include <armadillo>
#define RCPP_STOP(msg) throw std::runtime_error(msg)
#define RCPP_RCERR std::cerr
#else
#include <RcppArmadillo.h>
#define RCPP_STOP(msg) RCPP_STOP(msg)
#define RCPP_RCERR RCPP_RCERR
#endif
#include <bitset>
#include <iostream>
#include <sstream>
...
void Simulator::readInputParameters(CommandArguments arguments){
    ...
    try{
      ...
      case 'F':
        if (arguments[iCurrentArg].size()!=3){
          RCPP_RCERR<<"For the SNP ascertainment feature you must enter the filename of the SNP "<<
      ...
      case 'c' :
        ...
        if (pConfig->dGeneConvRatio<0){
          RCPP_RCERR<<"The gene conversion parameters must be positive\n";
          RCPP_STOP("Argument error");
        }
        break;
      case 'R':
        if (arguments[iCurrentArg].size()!=2){
          RCPP_RCERR<<"For the hotspot feature you must enter the filename of the hotspot list."<<endl;
          RCPP_STOP("Argument error");
        }
        filename = arguments[iCurrentArg][1].data();
        inFile.open(filename);
      ...
      case 'I' :
        ...
        if (arguments[iCurrentArg].size()==iNoMigrPops||
          ...
          for(unsigned int i=0; i<pConfig->iTotalPops; ++i) {
            ...
            //RCPP_RCERR<<"INPUT: Setting chr sampled for pop "<<(i+1)<<" to "<<newPop.getChrSampled()<<endl;
            newPop.setPopSize(dDefaultPopSize) ;
            newPop.setGrowthAlpha(dDefaultGrowthAlpha);
      ...
      case 'm' :
        if( pConfig->iTotalPops < 2 ) {
          RCPP_RCERR<<"You must use -I option first (i.e. specify more than one population)."<<endl;
          RCPP_STOP("Argument error");
        }
        if (arguments[iCurrentArg][0][2]=='a') {
          ...
          if (arguments[iCurrentArg].size()!=iTotalCells){
            RCPP_RCERR<<"For flag "<<arguments[iCurrentArg][0]<<
      ...
      case 'n' :
        ...
        if (arguments[iCurrentArg].size()!=3){
          ...
          if (popId>pConfig->iTotalPops){
            RCPP_RCERR<<"Invalid pop ID"<<endl;
            RCPP_STOP("Argument error");
          }
      ...
      case 'G' :
        ...
        if (arguments[iCurrentArg].size()!=2){
          ...
          //                    RCPP_RCERR<<"INPUT: Growth rate for all pop "<<dDefaultGrowthAlpha<<endl;
          for(int i=0; i<pConfig->iTotalPops; ++i){
      ...
      case 'e' :
        ...
        if (arguments[iCurrentArg].size()<2){
          RCPP_RCERR<<"For event flags, you need to specify at least a time after "<<
        ...
        if (eventTimes.find(dTime)==eventTimes.end()){
          ...
          RCPP_RCERR<<"Error, this event is redundant with a previous time.  Please increment it slightly from "<<dTime<<" to prevent unpredictable results\n";
          throw "Invalid input";
        }
        int iPop1,iPop2;
        double dProportion;
        ...
        case 'G': // global growth rate
          if (arguments[iCurrentArg].size()!=3){
            ...
            wrapper = EventPtr(new GenericEvent(
              ...
              g));
            //RCPP_RCERR<<"Global growth rate is "<<
            //  g<<endl;
          }
          break;
        case 'M': // global migration rate
          ...
          if (arguments[iCurrentArg].size()!=3){
            ...
            wrapper = EventPtr(new GenericEvent(
              ...
              atof(arguments[iCurrentArg][2].data())));
            //RCPP_RCERR<<"Global migration rate is "<<
            //  atof( arguments[iCurrentArg][2].data() )<<endl;
          }
          break;
        case 'n' :  // subpopulation size
          if (arguments[iCurrentArg].size()!=4){
            ...
            //RCPP_RCERR<<"For population "<<arguments[iCurrentArg][2]<<
        ...
        case 's' :  // split
          ...
          if (arguments[iCurrentArg].size()!=4){
            ...
            //RCPP_RCERR<<"Population "<<arguments[iCurrentArg][2]<<
        ...
        case 'j':   // move lineages from pop1 to pop2
          ...
          if (arguments[iCurrentArg].size()!=4){
            ...
            if (iPop1>=pConfig->iTotalPops||
              ...
              RCPP_RCERR<<"WARNING: The pop IDs used in pop join is greater than the number specified in -I.  You must have a split event before this join event.\n";
            }
            wrapper = EventPtr(new PopJoinEvent(
              Event::POPJOIN,dTime,iPop1,iPop2));
            //RCPP_RCERR<<"Population "<<
        ...
        case 'm':
          ...
          if (bAcceptFullMigrMatrix){ // the -ema iTotalPops
            ...
            wrapper = EventPtr(new
                                 MigrationRateMatrixEvent(
                                   ...
                                   dMigrationMatrix));
            //RCPP_RCERR<<"Full migration matrix provided by the user\n";
            ...
            if (arguments[iCurrentArg].size()!=5){
              ...
              //RCPP_RCERR<<"Mig rate of source pop "<<arguments[iCurrentArg][2] <<" to dest pop "<<arguments[iCurrentArg][3]<<" set to "<<arguments[iCurrentArg][4]<<".\n";
          ...
          break;
        default:
          RCPP_RCERR<<"Invalid suboption, you entered"<<chType<<endl;
        break;
        }
        pEventList->push_back(wrapper);
        break;
      default:
        RCPP_RCERR<<"Invalid option, you entered "<<arguments[iCurrentArg][0][1]<<endl;
      }
    }catch(const out_of_range & e){
      RCPP_RCERR<<"There were too many arguments.\n";
    }
  }
  ...
  if (pConfig->iGeneConvTract>pConfig->dBasesToTrack) {
    RCPP_RCERR<<"Warning: the gene conversion tract (-c 2nd parameter) cannot be "<<
      "longer than the length of sequence (-h parameter) to retain. ";
    pConfig->dBasesToTrack=2.0*pConfig->iGeneConvTract;
    //RCPP_RCERR<<"The -h parameter is now revised to the recommend value of 2*tractlen = "
...
vector<AlphaSimRReturn> Simulator::beginSimulationMemory() {
  ...
  try {
    ...
    RCPP_RCERR << "Simulator caught exception with message:" << endl << message << endl;
  }
  return  toRet;
}
...
#else
    RCPP_STOP("Not enough args for macs call");
#endif
  }
  if (in.empty()) {
#ifdef ALPHASIMPY_PYTHON
