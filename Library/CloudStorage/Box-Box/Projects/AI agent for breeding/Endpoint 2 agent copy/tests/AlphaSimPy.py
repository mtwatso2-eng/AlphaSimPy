...
def as_categorical(x: np.ndarray, p: Optional[Union[float, np.ndarray, List]] = None,
    ...
    if p is not None:
        ...
        if np.isscalar(p[0]) and p[0] == 0.5 and n_traits == 1:
            warnings.warn("p=0.5 (scalar) - use array [0.5, 0.5] for two equal categories")
        threshold = []
...
def make_cross(pop: Pop, cross_plan: np.ndarray, n_progeny: int = 1,
    ...
    if cross_plan.dtype.kind in ('U', 'S', 'O'):
        f_idx = np.array([pop.id.index(str(r[0])) for r in cross_plan])
        m_idx = np.array([pop.id.index(str(r[1])) for r in cross_plan])
...
def make_cross2(females: Pop, males: Pop, cross_plan: np.ndarray, n_progeny: int = 1,
    ...
    cross_plan = np.asarray(cross_plan)
    if cross_plan.dtype.kind in ('U', 'S', 'O'):
        f_idx = np.array([females.id.index(str(r[0])) for r in cross_plan])
        m_idx = np.array([males.id.index(str(r[1])) for r in cross_plan])
        cross_plan = np.column_stack([f_idx, m_idx])
    else:
        cross_plan = cross_plan.astype(int)
        if np.min(cross_plan) >= 1:
